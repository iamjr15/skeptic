/**
 * AI-driven *.spec.ts generator. The pre-pivot version emitted YAML and
 * Zod-validated each chunk; B5.5 rewrites the validation against the TS
 * compiler API: write the LLM output to a temp `.spec.ts` file, typecheck
 * it via `typecheckSpecs` (the same path `mcp validate_tests` uses), and
 * sanity-check that ≥1 `test(...)` call is registered.
 *
 * The function names (`generateFromDiff`, `generateFromDescription`) are
 * kept for git continuity — `cli/src/commands/generate.ts` and the test
 * suite both import them by name.
 */
