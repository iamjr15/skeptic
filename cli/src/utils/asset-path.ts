...
export const readTemplate = (relPath: string): Buffer => {
  ...
  return readFileSync(join(getTemplatesDir(), relPath));
};

/**
 * Path to the recorder script that gets injected into Playwright via
 * `addInitScript({ path })`. In SEA mode, the asset bytes live inside the
 * binary, but Playwright reads from disk — so we extract once per process to
 * a private mkdtemp directory and return that path.
 */
...
export const getRecorderScript = (): string => {
  if (isSea()) {
    ...
    const getAsset = nodeSea.getAsset;
    if (!getAsset) throw new Error("getRecorderScript: SEA detected but getAsset unavailable");
    const blob = getAsset("recorder-script.js");
    if (!blob) throw new Error("getRecorderScript: missing SEA asset recorder-script.js");
    // Use mkdtemp so the path is unguessable (vs the predictable
    // `skeptic-recorder-${pid}.js` we'd otherwise create in os.tmpdir() — that
    // pattern is a known POSIX vulnerability vector for pre-creation attacks).
    // Mode 0700 on POSIX (Node default for mkdtemp); file written 0600.
    const dir = mkdtempSync(join(os.tmpdir(), "skeptic-rec-"));
    const filePath = join(dir, "recorder-script.js");
    writeFileSync(filePath, Buffer.from(blob), { mode: 0o600 });
  ...
  // Production (npm bundle): dist/skeptic.mjs lives next to dist/recorder-script.js.
  const bundled = join(here, "recorder-script.js");
  if (existsSync(bundled)) return bundled;

  // Dev: cli/src/utils/asset-path.ts → ../commands/recorder-script.js.
  return resolve(here, "../commands/recorder-script.js");
};
...
export const getWebVitalsIifeScript = (): string => {
  if (isSea()) {
    ...
    const dir = mkdtempSync(join(os.tmpdir(), "skeptic-wv-"));
