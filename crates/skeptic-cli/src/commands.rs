...
pub fn parse_command(args: &[String], flags: &Flags) -> Result<Value, ParseError> {
    ...
    // Inject SKEPTIC_DEFAULT_TIMEOUT into any wait-family command that
...
fn parse_command_inner(args: &[String], flags: &Flags) -> Result<Value, ParseError> {
    ...
    match cmd {
        ...
        "audit" | "a11y" => {
                ...
                match rest[i] {
                            ...
                            Some(
                                *rest
                                    ...
                                    .ok_or_else(|| ParseError::MissingArguments {
                                        context: "audit --selector".to_string(),
                                        usage: "audit [--selector <css>] [--tags <tag,...>]",
                                    })?,
                            );
                        i += 1;
                    }
                    ...
                    value => {
                        return Err(ParseError::InvalidValue {
                            message: format!("unknown audit argument `{value}`"),
                            usage: "audit [--selector <css>] [--tags <tag,...>]",
                        });
                    }
                }
            ...
            let mut command = json!({"id": id, "action": "audit"});
            if let Some(selector) = selector {
                command["selector"] = json!(selector);
            }
            if !tags.is_empty() {
                command["tags"] = json!(tags);
            }
            Ok(command)
        }

        "visual" => parse_visual(&rest, &id),
            ...
            match sub {
                Some("save") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "skeptic auth save <name> --url <url> --username <user> --password <pass>",
                    })?;
                    ...
                    let url_val = url.ok_or_else(|| ParseError::MissingArguments {
                        context: "auth save".to_string(),
                        usage: "skeptic auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
                    })?;
                ...
                Some("login") => {
                    const AUTH_LOGIN_USAGE: &str = "skeptic auth login <name> [--credential-provider <plugin>] [--item <ref>] [--url <url>] [--username-selector <s>] [--password-selector <s>] [--submit-selector <s>]";
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                ...
                Some("delete") | Some("remove") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "skeptic auth delete <name>",
                    })?;
        ...
        "confirm" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "skeptic confirm <confirmation-id>",
            })?;
        ...
        "deny" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "skeptic deny <confirmation-id>",
            })?;
...
fn parse_visual(rest: &[&str], id: &str) -> Result<Value, ParseError> {
    let operation = rest
        .first()
        .copied()
        .ok_or_else(|| ParseError::MissingArguments {
            context: "visual".to_string(),
            usage: "visual <check|update|status> [name] [options]",
        })?;
    if operation == "status" {
        if rest.len() > 1 {
            return Err(ParseError::InvalidValue {
                message: "visual status takes no positional arguments".to_string(),
                usage: "visual status",
            });
        }
        return Ok(json!({"id": id, "action": "visual_status"}));
    }
    if !matches!(operation, "check" | "compare" | "update") {
        return Err(ParseError::UnknownSubcommand {
            subcommand: operation.to_string(),
            valid_options: &["check", "update", "status"],
        });
    }
    let name = rest.get(1).filter(|value| !value.starts_with('-')).ok_or_else(|| {
        ParseError::MissingArguments {
            context: format!("visual {operation}"),
            usage: "visual <check|update> <name> [--selector <css>] [--mask <css>] [--threshold <0..1>] [--full]",
        }
    })?;
    let operation = if operation == "compare" {
        "check"
    } else {
        operation
    };
    let mut command = json!({"id": id, "action": "visual", "operation": operation, "name": name});
    let mut masks = Vec::new();
    let mut i = 2;
    while i < rest.len() {
        match rest[i] {
            "--selector" | "-s" => {
                command["selector"] = json!(rest.get(i + 1).ok_or_else(|| ParseError::MissingArguments {
                    context: "visual --selector".to_string(), usage: "visual check <name> --selector <css>",
                })?);
                i += 1;
            }
            "--mask" => {
                masks.push(*rest.get(i + 1).ok_or_else(|| ParseError::MissingArguments {
                    context: "visual --mask".to_string(), usage: "visual check <name> --mask <css>",
                })?);
                i += 1;
            }
            "--threshold" => {
                let raw = rest.get(i + 1).ok_or_else(|| ParseError::MissingArguments {
                    context: "visual --threshold".to_string(), usage: "visual check <name> --threshold <0..1>",
                })?;
                let threshold = raw.parse::<f64>().map_err(|_| ParseError::InvalidValue {
                    message: format!("invalid visual threshold `{raw}`"), usage: "visual check <name> --threshold <0..1>",
                })?;
                if !(0.0..=1.0).contains(&threshold) {
                    return Err(ParseError::InvalidValue { message: "visual threshold must be between 0 and 1".to_string(), usage: "visual check <name> --threshold <0..1>" });
                }
                command["threshold"] = json!(threshold);
                i += 1;
            }
            "--full" | "-f" => command["fullPage"] = json!(true),
            value => return Err(ParseError::InvalidValue {
                message: format!("unknown visual argument `{value}`"),
                usage: "visual <check|update> <name> [--selector <css>] [--mask <css>] [--threshold <0..1>] [--full]",
            }),
        }
        i += 1;
...
mod tests {
    ...
    #[test]
    fn removed_dashboard_and_stream_commands_stay_removed() {
        for command in ["dashboard start", "stream enable"] {
            let result = parse_command(&args(command), &default_flags());
    ...
    // === Default timeout (SKEPTIC_DEFAULT_TIMEOUT) tests ===
