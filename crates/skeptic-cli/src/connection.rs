...
pub(crate) const INTERNAL_DAEMON_SHUTDOWN_ACTION: &str = "__skeptic_internal_shutdown";
...
/// Priority: SKEPTIC_SOCKET_DIR > XDG_RUNTIME_DIR > ~/.skeptic > tmpdir
pub fn get_socket_dir() -> PathBuf {
    // 1. Explicit override (ignore empty string)
    let base = if let Ok(dir) = env::var("SKEPTIC_SOCKET_DIR") {
        if !dir.is_empty() {
            PathBuf::from(dir)
        } else if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
            if !runtime_dir.is_empty() {
                PathBuf::from(runtime_dir).join("skeptic")
            } else if let Some(home) = dirs::home_dir() {
                home.join(".skeptic")
            } else {
                env::temp_dir().join("skeptic")
            }
        } else if let Some(home) = dirs::home_dir() {
            home.join(".skeptic")
        } else {
    ...
    if let Ok(namespace) = env::var("SKEPTIC_NAMESPACE") {
...
fn apply_daemon_env(cmd: &mut Command, session: &str, opts: &DaemonOptions) {
    cmd.env("SKEPTIC_DAEMON", "1")
        .env("SKEPTIC_SESSION", session);

    if opts.headed {
        cmd.env("SKEPTIC_HEADED", "1");
    }
    if opts.debug {
        cmd.env("SKEPTIC_DEBUG", "1");
    }
    if let Some(path) = opts.executable_path {
        cmd.env("SKEPTIC_EXECUTABLE_PATH", path);
    }
    if !opts.extensions.is_empty() {
        cmd.env("SKEPTIC_EXTENSIONS", opts.extensions.join(","));
    }
    if !opts.init_scripts.is_empty() {
        cmd.env("SKEPTIC_INIT_SCRIPTS", opts.init_scripts.join(","));
    }
    if !opts.enable.is_empty() {
        cmd.env("SKEPTIC_ENABLE", opts.enable.join(","));
    }
    if let Some(a) = opts.args {
        cmd.env("SKEPTIC_ARGS", a);
    }
    if let Some(ua) = opts.user_agent {
        cmd.env("SKEPTIC_USER_AGENT", ua);
    }
    if let Some(p) = opts.proxy {
        cmd.env("SKEPTIC_PROXY", p);
    }
    if let Some(pb) = opts.proxy_bypass {
        cmd.env("SKEPTIC_PROXY_BYPASS", pb);
    }
    if let Some(pu) = opts.proxy_username {
        cmd.env("SKEPTIC_PROXY_USERNAME", pu);
    }
    if let Some(pp) = opts.proxy_password {
        cmd.env("SKEPTIC_PROXY_PASSWORD", pp);
    }
    if opts.ignore_https_errors {
        cmd.env("SKEPTIC_IGNORE_HTTPS_ERRORS", "1");
    }
    if opts.allow_file_access {
        cmd.env("SKEPTIC_ALLOW_FILE_ACCESS", "1");
    }
    cmd.env(
    ...
    if opts.webgpu {
        cmd.env("SKEPTIC_WEBGPU", "1");
    }
    if let Some(prof) = opts.profile {
        cmd.env("SKEPTIC_PROFILE", prof);
    }
    if let Some(st) = opts.state {
        cmd.env("SKEPTIC_STATE", st);
    }
    if let Some(p) = opts.provider {
        cmd.env("SKEPTIC_PROVIDER", p);
    }
    if let Some(d) = opts.device {
        cmd.env("SKEPTIC_IOS_DEVICE", d);
    }
    if let Some(sn) = opts.session_name {
        cmd.env("SKEPTIC_SESSION_NAME", sn);
    }
    if let Some(policy) = opts.restore_save {
        cmd.env("SKEPTIC_RESTORE_SAVE", policy);
    }
    if let Some(check) = opts.restore_check_url {
        cmd.env("SKEPTIC_RESTORE_CHECK_URL", check);
    }
    if let Some(check) = opts.restore_check_text {
        cmd.env("SKEPTIC_RESTORE_CHECK_TEXT", check);
    }
    if let Some(check) = opts.restore_check_fn {
        cmd.env("SKEPTIC_RESTORE_CHECK_FN", check);
    }
    if let Some(dp) = opts.download_path {
        cmd.env("SKEPTIC_DOWNLOAD_PATH", dp);
    }
    if let Some(ad) = opts.allowed_domains {
        cmd.env("SKEPTIC_ALLOWED_DOMAINS", ad.join(","));
    }
    if let Some(ap) = opts.action_policy {
        cmd.env("SKEPTIC_ACTION_POLICY", ap);
    }
    if let Some(ca) = opts.confirm_actions {
        cmd.env("SKEPTIC_CONFIRM_ACTIONS", ca);
    }
    if let Some(engine) = opts.engine {
        cmd.env("SKEPTIC_ENGINE", engine);
    }
    if opts.auto_connect {
        cmd.env("SKEPTIC_AUTO_CONNECT", "1");
    }
    if let Some(idle) = opts.idle_timeout {
        cmd.env("SKEPTIC_IDLE_TIMEOUT_MS", idle);
    }
    if let Some(timeout) = opts.default_timeout {
        cmd.env("SKEPTIC_DEFAULT_TIMEOUT", timeout.to_string());
    }
    if let Some(cdp) = opts.cdp {
        cmd.env("SKEPTIC_CDP", cdp);
    }
    if opts.no_auto_dialog {
        cmd.env("SKEPTIC_NO_AUTO_DIALOG", "1");
    }
    if let Some(plugins) = opts.plugins {
        cmd.env("SKEPTIC_PLUGINS", plugins);
    }
}
...
fn concurrent_daemon_config_error(session: &str) -> String {
    format!(
        "A daemon for session '{}' started concurrently with different daemon configuration. Retry the command so skeptic can restart it with the requested configuration.",
        session
    )
}
...
pub fn ensure_daemon(session: &str, opts: &DaemonOptions) -> Result<DaemonResult, String> {
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&socket_dir, fs::Permissions::from_mode(0o700))
            .map_err(|error| format!("Failed to secure socket directory: {error}"))?;
    }

    // Serialize capacity admission through daemon startup. Without this
    // lock, parallel runner workers can all observe the same free slot and
    // exceed the configured per-namespace session cap.
    let capacity_lock_path = socket_dir.join(".capacity.lock");
    ...
    let capacity_lock: File = lock_options
        .open(&capacity_lock_path)
        .map_err(|error| format!("cannot open {}: {error}", capacity_lock_path.display()))?;
    capacity_lock
        .lock_exclusive()
        .map_err(|error| format!("cannot lock {}: {error}", capacity_lock_path.display()))?;

    let max_sessions = env::var("SKEPTIC_MAX_SESSIONS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(8);
    let active_sessions = walk_daemons().sessions.len();
    skeptic_daemon::enforce_session_capacity(active_sessions, max_sessions)
        ...
        if path_len > 103 {
            return Err(format!(
                "Session name '{}' is too long. Socket path would be {} bytes (max 103).\n\
                 Use a shorter session name or set SKEPTIC_SOCKET_DIR to a shorter path.",
                session, path_len
            ));
        }
    }
...
/// parse_command stamps with SKEPTIC_DEFAULT_TIMEOUT when no explicit
...
fn send_command_once(cmd: &Value, session: &str) -> Result<Response, String> {
    ...
    let request = authenticated_command(cmd, session)?;
    let mut json_str = serde_json::to_string(&request).map_err(|e| e.to_string())?;
    json_str.push('\n');
...
fn authenticated_command(cmd: &Value, session: &str) -> Result<Value, String> {
    #[cfg(windows)]
    {
        let token_path = get_token_path(session);
        let token = fs::read_to_string(&token_path).map_err(|error| {
            format!(
                "Failed to read daemon authentication token {}: {error}",
                token_path.display()
            )
        })?;
...
mod tests {
    ...
    fn test_get_socket_dir_explicit_override() {
        let _guard = EnvGuard::new(&["SKEPTIC_SOCKET_DIR", "XDG_RUNTIME_DIR"]);

        _guard.set("SKEPTIC_SOCKET_DIR", "/custom/socket/path");
    ...
    fn test_get_socket_dir_ignores_empty_socket_dir() {
        ...
        _guard.set("SKEPTIC_SOCKET_DIR", "");
        _guard.remove("XDG_RUNTIME_DIR");

        assert!(get_socket_dir().to_string_lossy().ends_with(".skeptic"));
    }
    ...
    fn test_get_socket_dir_xdg_runtime() {
        ...
        _guard.remove("SKEPTIC_SOCKET_DIR");
        _guard.set("XDG_RUNTIME_DIR", "/run/user/1000");

        assert_eq!(get_socket_dir(), PathBuf::from("/run/user/1000/skeptic"));
    }
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().ends_with(".skeptic"));
        assert!(
    ...
    fn test_get_socket_dir_namespace_scopes_base_directory() {
        let _guard = EnvGuard::new(&["SKEPTIC_SOCKET_DIR", "XDG_RUNTIME_DIR", "SKEPTIC_NAMESPACE"]);

        _guard.set("SKEPTIC_SOCKET_DIR", "/tmp/skeptic-test-sockets");
        _guard.remove("XDG_RUNTIME_DIR");
        _guard.set("SKEPTIC_NAMESPACE", "Worktree: One");

        assert_eq!(
            get_socket_dir(),
            PathBuf::from("/tmp/skeptic-test-sockets")
                .join("namespaces")
                .join("worktree-one")
                .join("run")
        );
    ...
    fn test_walk_daemons_only_lists_current_namespace() {
        ...
        _guard.set("SKEPTIC_SOCKET_DIR", dir.path().to_str().unwrap());
        _guard.remove("XDG_RUNTIME_DIR");
        ...
        _guard.set("SKEPTIC_NAMESPACE", "one");
    ...
    fn test_spawn_race_loser_does_not_overwrite_winner_config() {
        let guard = EnvGuard::new(&["SKEPTIC_SOCKET_DIR", "SKEPTIC_NAMESPACE"]);
        let dir = tempfile::tempdir().unwrap();
    ...
    fn test_get_port_for_session() {
        let guard = EnvGuard::new(&["SKEPTIC_NAMESPACE"]);
        guard.remove("SKEPTIC_NAMESPACE");
    ...
    fn test_get_port_for_session_includes_namespace() {
        ...
        guard.set("SKEPTIC_NAMESPACE", "Worktree: Two");
    ...
    fn test_daemon_version_matches_same_version() {
        ...
        let _guard = EnvGuard::new(&["SKEPTIC_SOCKET_DIR", "XDG_RUNTIME_DIR"]);
        _guard.set("SKEPTIC_SOCKET_DIR", dir.to_str().unwrap());
