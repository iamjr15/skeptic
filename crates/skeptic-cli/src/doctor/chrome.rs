...
pub(super) fn check(checks: &mut Vec<Check>) {
    ...
    if let Ok(engine) = env::var("SKEPTIC_ENGINE") {
            ...
            if which_exists("lightpanda") {
                ...
                checks.push(
                    Check::new(
                        ...
                        category,
                        Status::Fail,
                        "SKEPTIC_ENGINE=lightpanda but no lightpanda binary on PATH",
                    )
                    .with_fix("install lightpanda or unset SKEPTIC_ENGINE"),
                );
            }
        }
    }
