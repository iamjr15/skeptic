...
//! only pixel values prove anything. Opt-in via `skeptic doctor
...
pub(super) fn check(checks: &mut Vec<Check>, opts: &DoctorOptions) {
    if env::var("SKEPTIC_PROVIDER").is_ok() {
        checks.push(Check::new(
            "webgpu.skipped.provider",
            CATEGORY,
            Status::Info,
            "Skipped (SKEPTIC_PROVIDER is set; would consume cloud quota)",
        ));
        return;
    }
    if env::var("SKEPTIC_CDP").is_ok() {
        checks.push(Check::new(
            "webgpu.skipped.cdp",
            CATEGORY,
            Status::Info,
            "Skipped (SKEPTIC_CDP is set; would attach to a real browser)",
        ));
        return;
    }
    ...
    let work_dir = env::temp_dir().join(format!("skeptic-doctor-webgpu-{}", uuid::Uuid::new_v4()));
    ...
    let daemon_opts = DaemonOptions {
        ...
        // SKEPTIC_ENGINE=lightpanda would otherwise test the wrong
    ...
    if let Err(e) = send_json(launch_cmd, &session) {
        checks.push(
            ...
            .with_fix("skeptic install   # or check --debug output"),
        );
        return;
    }
...
fn probe_fail(id: &str, message: String) -> Check {
    ...
    if cfg!(target_os = "linux") {
        ...
        check.with_fix("update Chrome to the latest stable (skeptic install)")
    }
}
