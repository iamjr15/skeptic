...
const CONFIG_DIR: &str = ".skeptic";
const CONFIG_FILENAME: &str = "config.json";
const PROJECT_CONFIG_FILENAME: &str = "skeptic.json";
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
    let explicit = extract_config_path(args)
        .map(|p| ("--config", p))
        .or_else(|| {
            env::var("SKEPTIC_CONFIG")
                .ok()
                .map(|p| ("SKEPTIC_CONFIG", Some(p)))
        });
    ...
    if let Some((source, maybe_path)) = explicit {
        ...
        // The v2 front controller validates and applies the canonical TOML
        // config before legacy browser flag parsing. This parser remains only
        // for temporary compatibility with the vendored browser engine.
...
pub struct Flags {
    ...
    pub webgpu: bool,
    /// Env-only (SKEPTIC_NO_XVFB): disable automatic Xvfb for headed
    ...
    pub screenshot_quality: Option<u32>,
    pub screenshot_format: Option<String>,
    pub idle_timeout: Option<String>, // Canonical milliseconds string for SKEPTIC_IDLE_TIMEOUT_MS
    pub default_timeout: Option<u64>, // SKEPTIC_DEFAULT_TIMEOUT in ms
...
pub fn parse_flags(args: &[String]) -> Flags {
    ...
    let extensions_env = env::var("SKEPTIC_EXTENSIONS")
        .ok()
        .map(|s| {
            s.split(',')
    ...
    let init_scripts_env = env::var("SKEPTIC_INIT_SCRIPTS")
        .ok()
        .map(|s| {
            s.split(',')
                .map(|p| p.trim().to_string())
    ...
    let enable_env = env::var("SKEPTIC_ENABLE")
        .ok()
        .map(|s| {
            s.split(',')
                .map(|p| p.trim().to_string())
    ...
    let plugins = env::var("SKEPTIC_PLUGINS")
        .ok()
        .and_then(
    ...
    let mut flags = Flags {
        json: env_var_is_truthy("SKEPTIC_JSON") || config.json.unwrap_or(false),
        format: env::var("SKEPTIC_FORMAT").unwrap_or_else(|_| "human".to_string()),
        output: env::var("SKEPTIC_OUTPUT").ok().map(PathBuf::from),
        idempotency_key: env::var("SKEPTIC_IDEMPOTENCY_KEY").ok(),
        headed: env_var_is_truthy("SKEPTIC_HEADED") || config.headed.unwrap_or(false),
        debug: env_var_is_truthy("SKEPTIC_DEBUG") || config.debug.unwrap_or(false),
        session: env::var("SKEPTIC_SESSION")
            .ok()
            .or(config.session)
            .unwrap_or_else(|| "default".to_string()),
        restore: env::var("SKEPTIC_RESTORE").ok().or(config.restore),
        restore_save: env::var("SKEPTIC_RESTORE_SAVE")
            .ok()
            .or(config.restore_save),
        restore_check_url: env::var("SKEPTIC_RESTORE_CHECK_URL")
            .ok()
            .or(config.restore_check_url),
        restore_check_text: env::var("SKEPTIC_RESTORE_CHECK_TEXT")
            .ok()
            .or(config.restore_check_text),
        restore_check_fn: env::var("SKEPTIC_RESTORE_CHECK_FN")
            .ok()
            .or(config.restore_check_fn),
        namespace: env::var("SKEPTIC_NAMESPACE").ok().or(config.namespace),
        restore_uses_session: false,
        headers: config.headers,
        executable_path: env::var("SKEPTIC_EXECUTABLE_PATH")
            .ok()
            .or(config.executable_path),
        cdp: env::var("SKEPTIC_CDP").ok().or(config.cdp),
        extensions,
        init_scripts,
        enable,
        profile: env::var("SKEPTIC_PROFILE").ok().or(config.profile),
        state: env::var("SKEPTIC_STATE").ok().or(config.state),
        proxy: env::var("SKEPTIC_PROXY")
            .ok()
            .or(config.proxy)
            ...
            .or_else(|| env::var("all_proxy").ok()),
        proxy_bypass: env::var("SKEPTIC_PROXY_BYPASS")
            .ok()
            .or(config.proxy_bypass)
            .or_else(|| env::var("NO_PROXY").ok())
            .or_else(|| env::var("no_proxy").ok()),
        args: env::var("SKEPTIC_ARGS").ok().or(config.args),
        user_agent: env::var("SKEPTIC_USER_AGENT").ok().or(config.user_agent),
        provider: env::var("SKEPTIC_PROVIDER").ok().or(config.provider),
        ignore_https_errors: env_var_is_truthy("SKEPTIC_IGNORE_HTTPS_ERRORS")
            || config.ignore_https_errors.unwrap_or(false),
        allow_file_access: env_var_is_truthy("SKEPTIC_ALLOW_FILE_ACCESS")
            || config.allow_file_access.unwrap_or(false),
        hide_scrollbars: env_var_bool("SKEPTIC_HIDE_SCROLLBARS")
            .or(config.hide_scrollbars)
            .unwrap_or(true),
        webgpu: env_var_is_truthy("SKEPTIC_WEBGPU") || config.webgpu.unwrap_or(false),
        no_xvfb: env_var_is_truthy("SKEPTIC_NO_XVFB"),
        device: env::var("SKEPTIC_IOS_DEVICE").ok().or(config.device),
        auto_connect: env_var_is_truthy("SKEPTIC_AUTO_CONNECT")
            || config.auto_connect.unwrap_or(false),
        session_name: env::var("SKEPTIC_SESSION_NAME")
            .ok()
            .or(config.session_name),
        annotate: env_var_is_truthy("SKEPTIC_ANNOTATE") || config.annotate.unwrap_or(false),
        color_scheme: env::var("SKEPTIC_COLOR_SCHEME")
            .ok()
            .or(config.color_scheme),
        download_path: env::var("SKEPTIC_DOWNLOAD_PATH")
            .ok()
            .or(config.download_path),
        content_boundaries: env_var_is_truthy("SKEPTIC_CONTENT_BOUNDARIES")
            || config.content_boundaries.unwrap_or(false),
        max_output: env::var("SKEPTIC_MAX_OUTPUT")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.max_output),
        allowed_domains: env::var("SKEPTIC_ALLOWED_DOMAINS")
            .ok()
            .map(|s| {
                s.split(',')
        ...
        action_policy: env::var("SKEPTIC_ACTION_POLICY")
            .ok()
            .or(config.action_policy),
        confirm_actions: env::var("SKEPTIC_CONFIRM_ACTIONS")
            .ok()
            .or(config.confirm_actions),
        confirm_interactive: env_var_is_truthy("SKEPTIC_CONFIRM_INTERACTIVE")
            || config.confirm_interactive.unwrap_or(false),
        engine: env::var("SKEPTIC_ENGINE").ok().or(config.engine),
        screenshot_dir: env::var("SKEPTIC_SCREENSHOT_DIR")
            .ok()
            .or(config.screenshot_dir),
        screenshot_quality: env::var("SKEPTIC_SCREENSHOT_QUALITY")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(config.screenshot_quality),
        screenshot_format: env::var("SKEPTIC_SCREENSHOT_FORMAT")
            .ok()
            .or(config.screenshot_format)
            .filter(|s| s == "png" || s == "jpeg"),
        idle_timeout: parse_idle_timeout_value(
            env::var("SKEPTIC_IDLE_TIMEOUT_MS").ok(),
            "SKEPTIC_IDLE_TIMEOUT_MS",
        )
        .or(config.idle_timeout),
        default_timeout: env::var("SKEPTIC_DEFAULT_TIMEOUT")
            .ok()
            .and_then(|s| s.parse::<u64>().ok()),
        no_auto_dialog: env_var_is_truthy("SKEPTIC_NO_AUTO_DIALOG")
        ...
        match arg {
            ...
            "--format" => {
                if let Some(value) = args.get(i + 1) {
                    flags.format = value.to_ascii_lowercase();
                    flags.json = flags.format == "json";
                    i += 1;
                }
            }
            s if s.starts_with("--format=") => {
                flags.format = s.trim_start_matches("--format=").to_ascii_lowercase();
                flags.json = flags.format == "json";
            }
            "--output" => {
                if let Some(value) = args.get(i + 1) {
            ...
            s if s.starts_with("--output=") => {
                flags.output = Some(PathBuf::from(s.trim_start_matches("--output=")));
            }
            "--idempotency-key" => {
                if let Some(value) = args.get(i + 1) {
                    flags.idempotency_key = Some(value.clone());
                    i += 1;
                }
            }
            s if s.starts_with("--idempotency-key=") => {
                flags.idempotency_key =
                    Some(s.trim_start_matches("--idempotency-key=").to_string());
            }
            "--headed" => {
                let (val, consumed) = parse_bool_arg(args, i);
                flags.headed = val;
    ...
    if !matches!(flags.format.as_str(), "human" | "json" | "ndjson") {
        eprintln!(
            "{} browser commands support --format human, json, or ndjson (got `{}`)",
...
pub fn clean_args(args: &[String]) -> Vec<String> {
    ...
    while i < args.len() {
        ...
        if arg.starts_with("--format=")
            ...
            || arg.starts_with("--idempotency-key=")
        {
            i += 1;
            continue;
        }
...
mod tests {
    ...
    fn test_config_deserialize_full() {
        let json = r#"{
            ...
            "plugins": [
                {
                    "name": "onepassword",
                    "command": "skeptic-plugin-1password",
                    "args": ["--account", "team"],
        ...
        assert_eq!(plugin.command, "skeptic-plugin-1password");
        assert_eq!(
            plugin.args,
    ...
    fn test_load_config_missing_file_returns_none() {
        let result = read_config_file(&PathBuf::from("/nonexistent/skeptic.json"));
    ...
    fn test_cdp_env_var_resolves_into_flags() {
        let guard = EnvGuard::new(&["SKEPTIC_CDP"]);
        guard.set("SKEPTIC_CDP", "9222");
        let flags = parse_flags(&args("open example.com"));
