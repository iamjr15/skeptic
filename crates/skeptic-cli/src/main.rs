...
use connection::{
    cleanup_stale_files, daemon_unreachable, ensure_daemon, get_socket_dir, send_command,
    walk_daemons, DaemonOptions, Response,
};
use flags::{clean_args, parse_flags, Flags};
use install::run_install;
use output::{
    exit_code_for_response, print_command_help, print_help, print_response_with_opts,
    print_version, OutputOptions,
};
...
fn incompatible_launch_mode_error(flags: &Flags) -> Option<&'static str> {
    ...
    // browser skeptic did not launch. Rejecting (rather than silently
...
fn run_interactive_confirmations(
    ...
    while let Some(prompt) = confirmation_prompt_from_response(&resp) {
        eprintln!("[skeptic] Action requires confirmation:");
        ...
        match send_command(confirm_cmd, &flags.session) {
            Ok(next_resp) => {
                if !approved {
                    ...
                    exit(exit_code_for_response(&resp));
                }
                resp = next_resp;
            }
            Err(e) => {
...
fn run_close_all(flags: &Flags) {
    // walk_daemons auto-cleans stale daemon sidecar files before returning
    // the active sessions that should receive `close`.
    let inventory = walk_daemons();
...
fn main() {
    ...
    // Native daemon mode: when SKEPTIC_DAEMON is set, run as the daemon process
    if env::var("SKEPTIC_DAEMON").is_ok() {
        ...
        let session = env::var("SKEPTIC_SESSION").unwrap_or_else(|_| "default".to_string());
    ...
    if let Some(code) = v2::try_run(&args) {
        exit(code);
    }
    ...
    let resolved_config = skeptic_config::load(
        &cwd,
        explicit_config.as_deref(),
        skeptic_config::ConfigOverrides::default(),
    )
    .unwrap_or_else(|error| {
        eprintln!("invalid Skeptic configuration: {error}");
        exit(2);
    });
    if env::var_os("SKEPTIC_IDLE_TIMEOUT_MS").is_none() {
        env::set_var(
            "SKEPTIC_IDLE_TIMEOUT_MS",
            resolved_config.config.session.idle_ttl_ms.to_string(),
        );
    }
    if env::var_os("SKEPTIC_MAX_SESSIONS").is_none() {
        env::set_var(
            "SKEPTIC_MAX_SESSIONS",
            resolved_config.config.session.max_sessions.to_string(),
        );
    }
    if env::var_os("SKEPTIC_DEFAULT_TIMEOUT").is_none() {
        env::set_var(
            "SKEPTIC_DEFAULT_TIMEOUT",
            resolved_config.config.runner.action_timeout_ms.to_string(),
        );
    }
    if env::var_os("SKEPTIC_ALLOWED_DOMAINS").is_none()
        && !resolved_config.config.policy.allowed_domains.is_empty()
    {
        env::set_var(
            "SKEPTIC_ALLOWED_DOMAINS",
            resolved_config.config.policy.allowed_domains.join(","),
        );
    }
    if env::var_os("SKEPTIC_VISUAL_MASKS").is_none() {
        env::set_var(
            "SKEPTIC_VISUAL_MASKS",
            serde_json::to_string(&resolved_config.config.visual.mask).unwrap_or_default(),
        );
    }
    if env::var_os("SKEPTIC_VISUAL_THRESHOLD").is_none() {
        env::set_var(
            "SKEPTIC_VISUAL_THRESHOLD",
            resolved_config.config.visual.threshold.to_string(),
        );
    }
    if resolved_config.config.visual.sensitive {
        env::set_var("SKEPTIC_VISUAL_SENSITIVE", "1");
    }
    let mut flags = parse_flags(&args);
    let _stdout_redirect = flags.output.as_ref().map(|path| {
        ...
        let file = fs::File::create(path).unwrap_or_else(|error| {
            eprintln!("cannot create output file {}: {error}", path.display());
            exit(10);
        });
        gag::Redirect::stdout(file).unwrap_or_else(|error| {
            eprintln!("cannot redirect output to {}: {error}", path.display());
            exit(10);
        })
    });
    ...
    if let Some(ref namespace) = flags.namespace {
        env::set_var("SKEPTIC_NAMESPACE", namespace);
    }
    ...
    if clean.first().map(|s| s.as_str()) == Some("doctor") {
        let opts = doctor::DoctorOptions {
            ...
            json: flags.json,
            // Explicit CLI opt-in only: a global SKEPTIC_WEBGPU/config
    ...
    if let Some(key) = flags.idempotency_key.as_ref() {
        cmd["idempotencyKey"] = json!(key);
    }
    ...
    if should_send_local_launch_config(&flags) {
        ...
        // absent, the daemon falls back to its spawn-time SKEPTIC_HEADED
        // env, so a follow-up command without --headed (common when env vars
        // like SKEPTIC_ARGS force a launch command on every call) does
...
mod tests {
    ...
    fn test_attach_plugins_to_command_adds_registry_payload() {
        let plugins = vec![crate::plugins::PluginConfig {
            ...
            command: "skeptic-plugin-stealth".to_string(),
    ...
    fn launch_mode_flags(auto_connect: bool, cdp: bool, provider: bool, extensions: bool) -> Flags {
        let mut flags = parse_flags(&[]);
        // Deterministic regardless of ambient SKEPTIC_WEBGPU.
        flags.webgpu = false;
