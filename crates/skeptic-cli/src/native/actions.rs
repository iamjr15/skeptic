...
use skeptic_evidence::{action_payload, session_journal_path, Journal};
...
struct DrainedEvents {
    ...
    /// pauses them, even though skeptic does not track them as tabs.
...
pub struct DaemonState {
    ...
    pub engine: String,
    /// Default timeout for wait operations, from SKEPTIC_DEFAULT_TIMEOUT env var.
    ...
    /// Durable session event stream shared by interactive commands and `skeptic mark`.
    journal: Journal,
    /// Per-session bounded replay cache for retry-safe mutating commands.
    idempotency_cache: HashMap<String, Value>,
    idempotency_order: VecDeque<String>,
}

fn skeptic_project_root(start: &Path) -> PathBuf {
...
fn default_journal(session_id: &str) -> Journal {
    let cwd = env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    let root = skeptic_project_root(&cwd);
    let namespace = env::var("SKEPTIC_NAMESPACE").unwrap_or_else(|_| {
        let canonical = root.canonicalize().unwrap_or_else(|_| root.clone());
        hex::encode(Sha256::digest(canonical.to_string_lossy().as_bytes()))[..12].to_string()
    });
    Journal::for_session(
        session_journal_path(&root, &namespace, session_id),
        session_id,
    )
}
...
impl DaemonState {
    pub fn new() -> Self {
        let session_id = env::var("SKEPTIC_SESSION").unwrap_or_else(|_| "default".to_string());
        Self {
            browser: None,
            appium: None,
            safari_driver: None,
            ...
            domain_filter: Arc::new(RwLock::new(
                env::var("SKEPTIC_ALLOWED_DOMAINS")
                    .ok()
                    .filter(|s| !s.trim().is_empty())
                    .map(|s| DomainFilter::new(&s)),
            )),
            event_tracker: EventTracker::new(),
            session_name: env::var("SKEPTIC_SESSION_NAME").ok(),
            restore_save: env::var("SKEPTIC_RESTORE_SAVE")
                .ok()
                .unwrap_or_else(|| "auto".to_string()),
            restore_check_url: env::var("SKEPTIC_RESTORE_CHECK_URL").ok(),
            restore_check_text: env::var("SKEPTIC_RESTORE_CHECK_TEXT").ok(),
            restore_check_fn: env::var("SKEPTIC_RESTORE_CHECK_FN").ok(),
            ...
            auto_dialog: !matches!(
                env::var("SKEPTIC_NO_AUTO_DIALOG").as_deref(),
                Ok("1" | "true" | "yes")
            ),
            launch_hash: None,
            network_auto_attach_installed: false,
            engine: env::var("SKEPTIC_ENGINE").unwrap_or_else(|_| "chrome".to_string()),
            ...
            default_timeout_ms: env::var("SKEPTIC_DEFAULT_TIMEOUT")
                .ok()
            ...
            journal: default_journal(&session_id),
    ...
    /// configured `default_timeout_ms` (from `SKEPTIC_DEFAULT_TIMEOUT`).
        ...
        loop {
            match rx.try_recv() {
                ...
                Err(broadcast::error::TryRecvError::Lagged(n)) => {
                    eprintln!("[skeptic] Warning: CDP event buffer overflowed, {} events dropped. Network requests may be missing from HAR output.", n);
                    continue;
                }
...
fn policy_actions_for_command(
    ...
    if action == "launch" {
        ...
        let plugins = plugins_from_command_or_env(cmd);
        let provider_launch = env::var("SKEPTIC_PROVIDER")
            .ok()
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    let correlation_id = cmd.get("id").and_then(Value::as_str).map(ToOwned::to_owned);
    ...
    if let Some(cached) = idempotency_key
        .as_ref()
        .and_then(|key| state.idempotency_cache.get(key))
    {
        return cached.clone();
    }
    ...
    let target = cmd
        ...
        .or_else(|| cmd.get("url"))
        .and_then(Value::as_str);
    let _ = state.journal.append(
        "action-start",
        action_payload(action, target, "started"),
    ...
    if success {
        if let Some(data) = response.get_mut("data").and_then(Value::as_object_mut) {
            data.entry("changed")
                .or_insert_with(|| json!(is_mutating_action(action)));
        }
    }
    let outcome = if success { "success" } else { "error" };
    let _ = state.journal.append(
        "action",
        action_payload(action, target, outcome),
        correlation_id,
    );
    if success && invalidates_refs(action) {
        let _ = state.journal.append(
            "ref-invalidation",
            json!({"reason": action, "scope": "session"}),
            cmd.get("id").and_then(Value::as_str).map(ToOwned::to_owned),
        );
    }

    if let Some(key) = idempotency_key {
        if state.idempotency_cache.len() >= 1024 {
            if let Some(expired) = state.idempotency_order.pop_front() {
                state.idempotency_cache.remove(&expired);
            }
        }
...
fn invalidates_refs(action: &str) -> bool {
    matches!(
        action,
        "snapshot"
            | "navigate"
...
async fn execute_command_inner(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    // Drain and apply pending CDP events (console, errors, and target lifecycle).
    ...
    // Check SKEPTIC_CONFIRM_ACTIONS (category-based, independent of policy file)
    ...
    let result = match action {
        ...
        "audit" => handle_audit(cmd, state).await,
        ...
        "visual" => handle_visual(cmd, state).await,
        "visual_status" => handle_visual_status(),
...
fn ensure_allowed_domains_supported_for_launch(
    ...
    {
        return Err(
            "--allowed-domains is not supported with --restore because saved state can replay origins before skeptic can verify they are in the allowlist"
                .to_string(),
        );
    }
...
async fn auto_launch(
    ...
    let engine = env::var("SKEPTIC_ENGINE").ok();
    ...
    if let Ok(cdp) = env::var("SKEPTIC_CDP") {
    ...
    if env::var("SKEPTIC_AUTO_CONNECT").is_ok() {
        ensure_allowed_domains_supported_for_launch(AllowedDomainsLaunchSupport {
            allowed_domains: &allowed_domains,
            cdp_url: None,
            cdp_port: None,
    ...
    // Cloud provider: when SKEPTIC_PROVIDER is set, connect via the
    ...
    if let Ok(provider) = env::var("SKEPTIC_PROVIDER") {
...
/// Apply SKEPTIC_ENABLE (built-in init scripts like `react-devtools`)
/// and SKEPTIC_INIT_SCRIPTS (user-provided files) to the browser so the
...
fn launch_options_from_env() -> LaunchOptions {
    ...
    let extensions: Option<Vec<String>> = env::var("SKEPTIC_EXTENSIONS").ok().map(|v| {
        v.split([',', '\n'])
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .collect()
    ...
    LaunchOptions {
        headless: !headed,
        executable_path: env::var("SKEPTIC_EXECUTABLE_PATH").ok(),
        proxy: env::var("SKEPTIC_PROXY").ok(),
        proxy_bypass: env::var("SKEPTIC_PROXY_BYPASS").ok(),
        proxy_username: env::var("SKEPTIC_PROXY_USERNAME").ok(),
        proxy_password: env::var("SKEPTIC_PROXY_PASSWORD").ok(),
        profile: env::var("SKEPTIC_PROFILE").ok(),
        allow_file_access: env::var("SKEPTIC_ALLOW_FILE_ACCESS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        args: env::var("SKEPTIC_ARGS")
            .map(|v| {
                v.split([',', '\n'])
                    .map(|s| s.trim().to_string())
                    .filter(|s| !s.is_empty())
            ...
            .unwrap_or_default(),
        extensions,
        storage_state: env::var("SKEPTIC_STATE").ok(),
        user_agent: env::var("SKEPTIC_USER_AGENT").ok(),
        ignore_https_errors: env::var("SKEPTIC_IGNORE_HTTPS_ERRORS")
            .map(|v| v == "1" || v == "true")
            .unwrap_or(false),
        color_scheme: env::var("SKEPTIC_COLOR_SCHEME").ok(),
        download_path: env::var("SKEPTIC_DOWNLOAD_PATH").ok(),
        hide_scrollbars: hide_scrollbars_from_env(),
        viewport_size: None,
        use_real_keychain: false,
        webgpu: webgpu_from_env(),
        no_xvfb: no_xvfb_from_env(),
        restrict_webrtc: env::var("SKEPTIC_ALLOWED_DOMAINS")
            .is_ok_and(|domains| !domains.trim().is_empty()),
    }
}

fn hide_scrollbars_from_env() -> bool {
    env::var("SKEPTIC_HIDE_SCROLLBARS")
...
fn headed_from_env() -> bool {
    env::var("SKEPTIC_HEADED")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false)
}

fn webgpu_from_env() -> bool {
    env::var("SKEPTIC_WEBGPU")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false)
}
...
/// Load storage state from SKEPTIC_STATE if set.
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let engine = cmd
        ...
        .and_then(|v| v.as_str())
        .map(String::from)
        .or_else(|| env::var("SKEPTIC_ENGINE").ok());
    let profile = cmd
        .get("profile")
        .and_then(|v| v.as_str())
        .map(String::from);
    ...
    let mut launch_options = LaunchOptions {
        ...
        executable_path: cmd
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| env::var("SKEPTIC_EXECUTABLE_PATH").ok()),
        ...
        proxy_username: cmd
            ...
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("SKEPTIC_PROXY_USERNAME").ok()),
        proxy_password: cmd
            .get("proxy")
            .and_then(|v| v.get("password"))
            .and_then(|v| v.as_str())
            .map(String::from)
            .or_else(|| env::var("SKEPTIC_PROXY_PASSWORD").ok()),
        profile,
...
async fn launch_ios(_cmd: &Value, _state: &mut DaemonState) -> Result<Value, String> {
    Err("The inherited Appium iOS provider is not part of Skeptic v2. Native iOS simulator support lands through the AXe/simctl driver in M5."
        .to_string())
}

async fn launch_safari(_cmd: &Value, _state: &mut DaemonState) -> Result<Value, String> {
    Err(
        "Safari/WebKit runtime coverage is deferred in Skeptic v2; use the CDP Chrome engine."
            .to_string(),
    )
}
...
const AXE_CORE_SOURCE: &str = include_str!("../../assets/a11y/axe.min.js");
const IBM_EQUAL_ACCESS_SOURCE: &str = include_str!("../../assets/a11y/ace.js");

/// Run both embedded accessibility engines and normalize their results into a
/// stable, agent-friendly shape. An engine failure is reported without hiding
/// results from the other engine.
async fn handle_audit(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
    if state.webdriver_backend.is_some() && state.browser.is_none() {
        return Err("Accessibility audit currently requires the CDP browser backend".to_string());
    }
    let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
    ...
    let mut engine_errors = serde_json::Map::new();
    let mut violations = Vec::new();

    if let Err(error) = mgr.evaluate(AXE_CORE_SOURCE, None).await {
        engine_errors.insert("axe-core".to_string(), json!(error));
    } else {
        ...
        let script = format!(
            r#"(async () => {{
              const result = await globalThis.axe.run({context}, {{runOnly: {{type: 'tag', values: {tags}}}}});
              return result.violations.map(v => ({{
                engine: 'axe-core', impact: v.impact || 'moderate', ruleId: v.id,
                description: v.description, help: v.help, helpUrl: v.helpUrl,
                wcagTags: v.tags || [], nodes: v.nodes.map(n => ({{
                  selector: (n.target || []).join(' '), html: n.html || '',
                  failureSummary: n.failureSummary || ''
                }}))
              }}));
            }})()"#
        );
        match mgr.evaluate(&script, None).await {
            Ok(Value::Array(items)) => violations.extend(items),
            Ok(_) => {
                engine_errors.insert(
                    "axe-core".to_string(),
                    json!("engine returned an unexpected result"),
                );
            }
            Err(error) => {
                engine_errors.insert("axe-core".to_string(), json!(error));
    ...
    if let Err(error) = mgr.evaluate(IBM_EQUAL_ACCESS_SOURCE, None).await {
        engine_errors.insert("ibm-equal-access".to_string(), json!(error));
    } else {
        let target = selector
            .map(|value| {
                format!(
        ...
        let script = format!(
            r#"(async () => {{
              const checker = new globalThis.ace.Checker();
              const report = await checker.check({target}, ['WCAG_2_2']);
              return report.results
                .filter(r => r.value && r.value[0] === 'VIOLATION' && (r.value[1] === 'FAIL' || r.value[1] === 'POTENTIAL'))
                .map(r => ({{
                  engine: 'ibm-equal-access', impact: 'serious', ruleId: r.ruleId,
                  description: r.message, help: r.message,
                  helpUrl: `https://able.ibm.com/rules/archives/latest/doc/${{r.ruleId}}`,
                  wcagTags: [], nodes: [{{selector: r.path && r.path.dom || '',
                    html: r.snippet || '', failureSummary: r.message}}]
                }}));
            }})()"#
        );
        match mgr.evaluate(&script, None).await {
            Ok(Value::Array(items)) => {
                let axe_ids = violations
                    .iter()
                    .filter_map(|item| item.get("ruleId").and_then(Value::as_str))
                    .map(str::to_string)
                    .collect::<HashSet<_>>();
                violations.extend(items.into_iter().filter(|item| {
                    item.get("ruleId")
                        .and_then(Value::as_str)
                        .is_none_or(|id| !axe_ids.contains(id))
                }));
            }
            Ok(_) => {
                engine_errors.insert(
                    "ibm-equal-access".to_string(),
                    json!("engine returned an unexpected result"),
                );
            }
            Err(error) => {
    ...
    fn impact_rank(value: &Value) -> u8 {
        match value.get("impact").and_then(Value::as_str) {
            Some("critical") => 0,
            Some("serious") => 1,
            Some("moderate") => 2,
            _ => 3,
    ...
    violations.sort_by_key(impact_rank);
    let count = |impact: &str| {
        violations
            .iter()
            .filter(|violation| violation.get("impact").and_then(Value::as_str) == Some(impact))
            .count()
    };
    let origin = mgr.get_url().await.unwrap_or_default();
    let total = violations.len();
    Ok(json!({
        "schema": "skeptic.accessibility-audit/1",
        "origin": origin,
        "selector": selector,
        "engines": {
            "axe-core": {"version": "4.11.0", "ok": !engine_errors.contains_key("axe-core")},
            "ibm-equal-access": {"version": "4.0.16", "ok": !engine_errors.contains_key("ibm-equal-access")}
        },
        "engineErrors": engine_errors,
        "limitations": ["Cross-origin frames that deny script access are reported only when the engine can inspect them."],
        "violations": violations,
        "summary": {
            "total": total, "critical": count("critical"), "serious": count("serious"),
...
async fn handle_console(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    if clear {
        ...
        Ok(redact_console_value(result))
    }
}
...
async fn handle_session_info(state: &DaemonState) -> Result<Value, String> {
    Ok(json!({
        "session": state.session_id,
        "namespace": env::var("SKEPTIC_NAMESPACE").ok(),
...
fn write_extensions_file(session_id: &str) {
    if let Ok(val) = env::var("SKEPTIC_EXTENSIONS") {
...
async fn handle_getbyrole(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            for (const el of els) {{
                if ({name_match}) {{
                    el.setAttribute('data-skeptic-located', 'true');
                    return true;
                }}
            }}
            return false;
    ...
    let selector = "[data-skeptic-located='true']";
    ...
    if let Some(ref browser) = state.browser {
        if browser.active_session_id().is_ok() {
            let _ = browser
                .evaluate(
                    "document.querySelector('[data-skeptic-located]')?.removeAttribute('data-skeptic-located')",
                    None,
                )
                .await;
        }
    ...
    let query = match strategy {
        ...
        "label" => {
            ...
            format!(
                ...
                if (label) {{
                    ...
                    if (target) {{ target.setAttribute('data-skeptic-located', 'true'); return true; }}
                }}
                const aria = Array.from(document.querySelectorAll('[aria-label]')).find(el => matches(el.getAttribute('aria-label')));
                if (aria) {{ aria.setAttribute('data-skeptic-located', 'true'); return true; }}
                ...
                if (referenced) {{ referenced.setAttribute('data-skeptic-located', 'true'); return true; }}
                return false;
            }})()"#,
            )
        }
        "placeholder" => format!(
            r#"(() => {{
                ...
                if (el) {{ el.setAttribute('data-skeptic-located', 'true'); return true; }}
                return false;
            }})()"#,
...
async fn handle_nth(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            els[idx].setAttribute('data-skeptic-located', 'true');
            return true;
        }})()"#,
        sel = serde_json::to_string(selector).unwrap_or_default(),
        idx = index,
    ...
    let located = "[data-skeptic-located='true']";
...
fn visual_baseline_root() -> PathBuf {
    let cwd = env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    skeptic_project_root(&cwd).join("skeptic").join("baselines")
}

fn visual_safe_component(value: &str) -> String {
    let value = value
        .chars()
        .map(|character| {
...
fn collect_pngs(root: &Path, output: &mut Vec<String>) -> Result<(), String> {
    if !root.exists() {
        return Ok(());
    }
    for entry in
...
fn handle_visual_status() -> Result<Value, String> {
    let root = visual_baseline_root();
    let mut baselines = Vec::new();
    collect_pngs(&root, &mut baselines)?;
    baselines.sort();
    Ok(json!({
        "schema": "skeptic.visual-status/1", "root": root,
        "count": baselines.len(), "baselines": baselines
    }))
}

async fn handle_visual(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
    let operation = cmd
        .get("operation")
        .and_then(Value::as_str)
        .unwrap_or("check");
    ...
    let threshold = cmd
        ...
        .or_else(|| env::var("SKEPTIC_VISUAL_THRESHOLD").ok()?.parse().ok())
        .unwrap_or(0.1);
    let mut masks = env::var("SKEPTIC_VISUAL_MASKS")
        .ok()
    ...
    let (width, height, dpr, _) = state.viewport.unwrap_or((1280, 720, 1.0, false));
    let selector_hash = selector
        .as_deref()
        .map(|value| hex::encode(Sha256::digest(value.as_bytes()))[..8].to_string())
        .unwrap_or_else(|| "page".to_string());
    let environment_key = visual_safe_component(&format!(
        "{}-{}x{}@{}-{}-{}-{}",
        state.engine,
        width,
        height,
    ...
    let baseline_dir = visual_baseline_root().join(visual_safe_component(name));
    let baseline_path = baseline_dir.join(format!("{environment_key}.png"));
    let cwd = env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    let evidence_dir = skeptic_project_root(&cwd)
        .join(".skeptic")
        .join("visual")
        .join(visual_safe_component(name))
        .join(&environment_key);
    fs::create_dir_all(&evidence_dir)
        .map_err(|error| format!("cannot create {}: {error}", evidence_dir.display()))?;

    if !masks.is_empty() {
        let masks_json = serde_json::to_string(&masks).map_err(|error| error.to_string())?;
        let script = format!(
            r#"(() => {{
              document.getElementById('__skeptic_visual_masks__')?.remove();
              const root = document.createElement('div'); root.id = '__skeptic_visual_masks__';
              for (const selector of {masks_json}) {{
                for (const element of document.querySelectorAll(selector)) {{
                  const r = element.getBoundingClientRect(); const mask = document.createElement('div');
                  Object.assign(mask.style, {{position:'absolute', left:`${{r.left + scrollX}}px`, top:`${{r.top + scrollY}}px`,
                    width:`${{r.width}}px`, height:`${{r.height}}px`, background:'#000', zIndex:'2147483647', pointerEvents:'none'}});
                  root.appendChild(mask);
                }}
              }}
              document.documentElement.appendChild(root); return root.childElementCount;
            }})()"#
        );
    ...
    let capture_result: Result<(), String> = async {
        for index in 0..3 {
            let path = evidence_dir.join(format!("burst-{index}.png"));
            let result = screenshot::take_screenshot(
                &mgr.client,
                &session_id,
                &state.ref_map,
            ...
            captures.push(
                base64::Engine::decode(&base64::engine::general_purpose::STANDARD, result.base64)
                    .map_err(|error| format!("cannot decode visual capture: {error}"))?,
            );
            if index < 2 {
                tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
            }
    ...
    let stability = diff::diff_screenshot(&captures[1], &captures[2], threshold)?;
    let stable = stability.mismatch_percentage <= 0.1;
    let current = captures.pop().ok_or("visual capture produced no image")?;
    let current_path = evidence_dir.join("current.png");
    fs::write(&current_path, &current)
        .map_err(|error| format!("cannot write {}: {error}", current_path.display()))?;

    if operation == "update" {
        fs::create_dir_all(&baseline_dir)
            .map_err(|error| format!("cannot create {}: {error}", baseline_dir.display()))?;
        fs::write(&baseline_path, &current)
            .map_err(|error| format!("cannot write {}: {error}", baseline_path.display()))?;
        let metadata = json!({
            "schema": "skeptic.visual-baseline/1", "name": name, "key": environment_key,
            "selector": selector, "threshold": threshold, "updatedAt": OffsetDateTime::now_utc().format(&Rfc3339).unwrap_or_default()
        });
        fs::write(
            baseline_path.with_extension("json"),
            serde_json::to_vec_pretty(&metadata).map_err(|error| error.to_string())?,
        )
        .map_err(|error| error.to_string())?;
        return Ok(json!({
            "schema": "skeptic.visual-result/1", "name": name, "key": environment_key,
            "status": "updated", "match": true, "stable": stable,
            "baselinePath": baseline_path, "currentPath": current_path,
            "sensitivity": if env::var_os("SKEPTIC_VISUAL_SENSITIVE").is_some() {"sensitive"} else {"normal"}
        }));
    }
    ...
    if !baseline_path.is_file() {
        return Ok(json!({
            "schema": "skeptic.visual-result/1", "name": name, "key": environment_key,
            "status": "new", "match": false, "stable": stable,
            "baselinePath": baseline_path, "currentPath": current_path,
            "hint": format!("create the reviewed baseline with `skeptic visual update {name}`")
        }));
    }
    let baseline = fs::read(&baseline_path)
        .map_err(|error| format!("cannot read {}: {error}", baseline_path.display()))?;
    let comparison = diff::diff_screenshot(&baseline, &current, threshold)?;
    let diff_path = comparison
        .diff_image
        .as_ref()
        .map(|bytes| {
            let path = evidence_dir.join("diff.png");
            fs::write(&path, bytes)
                .map(|_| path)
                .map_err(|error| error.to_string())
        })
    ...
    Ok(json!({
        "schema": "skeptic.visual-result/1", "name": name, "key": environment_key,
        "status": if comparison.matched {"match"} else {"diff"}, "match": comparison.matched,
        "stable": stable, "mismatchPercentage": comparison.mismatch_percentage,
        "differentPixels": comparison.different_pixels, "totalPixels": comparison.total_pixels,
        "dimensionMismatch": comparison.dimension_mismatch, "baselinePath": baseline_path,
        "currentPath": current_path, "diffPath": diff_path,
        "sensitivity": if env::var_os("SKEPTIC_VISUAL_SENSITIVE").is_some() {"sensitive"} else {"normal"}
    }))
}
...
fn har_entry_to_json(e: HarEntry) -> Value {
    ...
    let request_cookies: Vec<Value> = e
        .request_headers
        .iter()
        .find(|(k, _)| k.eq_ignore_ascii_case("cookie"))
        .map(|(_, v)| {
            har_parse_request_cookies(v)
                .into_iter()
                .map(|mut cookie| {
    ...
    let query_string = har_parse_query_string(&redacted_url);

    let req_headers: Vec<Value> = e
        .request_headers
        .iter()
        .map(|(k, v)| json!({ "name": k, "value": redact_header(k, v) }))
        .collect();
    ...
    let resp_cookies: Vec<Value> = e
        ...
        .map(|(_, v)| {
            ...
            let _ = value;
            json!({ "name": name.trim(), "value": "[REDACTED]" })
        })
        .collect();
    ...
    if let Some(body) = e.post_data {
        let _ = body;
        request["postData"] = json!({ "mimeType": post_content_type, "text": "[REDACTED]" });
    }
...
fn redact_console_value(value: Value) -> Value {
    match value {
        ...
        Value::Array(values) => {
            Value::Array(values.into_iter().map(redact_console_value).collect())
        }
        Value::String(value) => {
            let bearer = regex::Regex::new(r"(?i)bearer\s+[a-z0-9._~+/-]+=*")
            ...
            Value::String(query.replace_all(&redacted, "$1[REDACTED]").into_owned())
        }
        value => value,
    }
}

fn redacted_request(request: &TrackedRequest) -> Value {
    let mut headers = request.headers.clone();
    if let Some(values) = headers.as_object_mut() {
    ...
    let mut response_headers = request.response_headers.clone();
    if let Some(values) = response_headers.as_mut().and_then(Value::as_object_mut) {
        for (name, value) in values.iter_mut() {
            if sensitive_field(name) {
                *value = json!("[REDACTED]");
            }
    ...
    json!({
        "url": redact_url(&request.url), "method": request.method, "headers": headers,
        "timestamp": request.timestamp, "resourceType": request.resource_type,
        "requestId": request.request_id,
        "postData": request.post_data.as_ref().map(|_| "[REDACTED]"),
        "status": request.status, "responseHeaders": response_headers,
        "mimeType": request.mime_type, "redaction": "redacted"
    })
}
...
fn get_har_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".skeptic").join("tmp").join("har")
    } else {
        std::env::temp_dir().join("skeptic").join("har")
    }
}
...
async fn handle_requests(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let document_url = if let Some(manager) = state.browser.as_ref() {
        manager.get_url().await.unwrap_or_default()
    } else {
        String::new()
    };
    let https_document = document_url.starts_with("https://");
    let mut issues = Vec::new();
    let mut prior: HashMap<(String, String), u64> = HashMap::new();
    for request in &requests {
        if request.status.is_some_and(|status| status >= 400) {
            issues.push(json!({
                "id": "network/http-error", "severity": if request.status.unwrap_or(0) >= 500 {"error"} else {"warning"},
                "message": format!("{} {} returned {}", request.method, redact_url(&request.url), request.status.unwrap_or(0)),
                "requestId": request.request_id, "status": request.status,
                "url": redact_url(&request.url)
            }));
        }
        let key = (request.method.to_ascii_uppercase(), request.url.clone());
        if let Some(previous) = prior.insert(key, request.timestamp) {
            if request.timestamp.saturating_sub(previous) < 500 {
                issues.push(json!({
                    "id": "network/duplicate-request", "severity": "warning",
                    "message": format!("Duplicate {} request within 500ms", request.method),
                    "requestId": request.request_id, "url": redact_url(&request.url),
                    "intervalMs": request.timestamp.saturating_sub(previous)
                }));
            }
        }
        if https_document && request.url.starts_with("http://") {
            issues.push(json!({
                "id": "network/mixed-content", "severity": "error",
                "message": "HTTPS page requested an insecure HTTP subresource",
                "requestId": request.request_id, "url": redact_url(&request.url)
            }));
        }
    }
    let redacted = requests
        .into_iter()
        .map(redacted_request)
        .collect::<Vec<_>>();
    Ok(json!({
        "schema": "skeptic.network-observation/1", "documentUrl": redact_url(&document_url),
        "requests": redacted, "issues": issues,
        "summary": {"requestCount": redacted.len(), "issueCount": issues.len()}
    }))
}
...
mod tests {
    ...
    fn unique_socket_dir(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("skeptic-{label}-{}-{nanos}", std::process::id()))
    }
    ...
    fn test_policy_actions_use_command_plugins_for_auto_launch_mutators() {
        let guard = EnvGuard::new(&["SKEPTIC_PROVIDER"]);
        guard.remove("SKEPTIC_PROVIDER");
        let cmd = json!({
            "action": "navigate",
            "id": "policy-plugin-1",
            ...
            "plugins": [
                {
                    "name": "stealth",
                    "command": "skeptic-plugin-stealth",
    ...
    fn test_policy_actions_use_command_plugins_for_provider_auto_launch() {
        let guard = EnvGuard::new(&["SKEPTIC_PROVIDER"]);
        guard.set("SKEPTIC_PROVIDER", "browserbox");
        let cmd = json!({
            "action": "navigate",
            "id": "policy-plugin-2",
            "url": "https://example.com",
            "plugins": [
                {
                    "name": "browserbox",
                    "command": "skeptic-plugin-browserbox",
...
printf invoked > "$1"
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"credential":{"username":"user","password":"pass","url":"https://example.com/login"}}'
"#,
        )
        .unwrap();
    ...
    async fn test_close_current_browser_closes_active_provider_plugin_session() {
        ...
        fs::write(
            ...
            r#"#!/bin/sh
cat > "$1"
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"data":{}}'
"#,
        )
        .unwrap();
        let mut perms = fs::metadata(&plugin_path).unwrap().permissions();
    ...
    async fn test_daemon_state_new() {
        let guard = EnvGuard::new(&[
            "SKEPTIC_ALLOWED_DOMAINS",
            "SKEPTIC_SESSION_NAME",
            "SKEPTIC_SESSION",
        ]);
        guard.remove("SKEPTIC_ALLOWED_DOMAINS");
        guard.remove("SKEPTIC_SESSION_NAME");
        guard.remove("SKEPTIC_SESSION");
    ...
    fn test_launch_options_from_env_defaults() {
        let guard = EnvGuard::new(&[
            "SKEPTIC_HEADED",
            "SKEPTIC_HIDE_SCROLLBARS",
            "SKEPTIC_ALLOWED_DOMAINS",
        ]);
        guard.remove("SKEPTIC_HEADED");
        guard.remove("SKEPTIC_HIDE_SCROLLBARS");
        guard.remove("SKEPTIC_ALLOWED_DOMAINS");
    ...
    fn test_launch_options_from_env_headed_flag() {
        let guard = EnvGuard::new(&["SKEPTIC_HEADED", "SKEPTIC_HIDE_SCROLLBARS"]);
        guard.set("SKEPTIC_HEADED", "1");
        guard.remove("SKEPTIC_HIDE_SCROLLBARS");
        let opts = launch_options_from_env();
        assert!(!opts.headless, "SKEPTIC_HEADED=1 should set headless=false");
    }

    #[test]
    fn test_launch_options_from_env_hide_scrollbars_false() {
        let guard = EnvGuard::new(&["SKEPTIC_HIDE_SCROLLBARS"]);
        guard.set("SKEPTIC_HIDE_SCROLLBARS", "false");
        let opts = launch_options_from_env();
        assert!(!opts.hide_scrollbars);
    }
    ...
    fn test_launch_options_from_env_webgpu() {
        let guard = EnvGuard::new(&["SKEPTIC_WEBGPU"]);
        guard.remove("SKEPTIC_WEBGPU");
        assert!(!launch_options_from_env().webgpu);
        guard.set("SKEPTIC_WEBGPU", "1");
        assert!(launch_options_from_env().webgpu);
    }
    ...
    fn test_no_xvfb_from_launch_cmd() {
        let guard = EnvGuard::new(&["SKEPTIC_NO_XVFB"]);
        guard.remove("SKEPTIC_NO_XVFB");
    ...
    fn test_allowed_domains_enable_webrtc_restriction_and_launch_hashing() {
        let guard = EnvGuard::new(&["SKEPTIC_ALLOWED_DOMAINS"]);
        guard.set("SKEPTIC_ALLOWED_DOMAINS", "example.com");
    ...
    async fn test_allowed_domains_reject_storage_state_replay() {
        ...
        let error = handle_launch(
            &json!({
                "action": "launch",
                "storageState": "/tmp/skeptic-state.json",
                "allowedDomains": ["example.com"]
            }),
            &mut state,
        )
    ...
    async fn test_allowed_domains_reject_profile_chrome_args() {
        ...
        let error = handle_launch(
            &json!({
                "action": "launch",
                "args": ["--user-data-dir=/tmp/skeptic-profile"],
                "allowedDomains": ["example.com"]
            }),
            &mut state,
        )
    ...
    async fn test_allowed_domains_reject_env_profile_during_auto_launch() {
        let guard = EnvGuard::new(&[
            ...
            "SKEPTIC_ARGS",
            "SKEPTIC_CDP",
            "SKEPTIC_AUTO_CONNECT",
            "SKEPTIC_PROVIDER",
        ]);
        guard.set("SKEPTIC_ALLOWED_DOMAINS", "example.com");
        guard.set("SKEPTIC_PROFILE", "/tmp/skeptic-profile");
        guard.remove("SKEPTIC_ARGS");
        guard.remove("SKEPTIC_CDP");
        guard.remove("SKEPTIC_AUTO_CONNECT");
        guard.remove("SKEPTIC_PROVIDER");
    ...
    async fn test_allowed_domains_reject_env_profile_args_during_auto_launch() {
        ...
        guard.set("SKEPTIC_ALLOWED_DOMAINS", "example.com");
        guard.remove("SKEPTIC_PROFILE");
        guard.set("SKEPTIC_ARGS", "--user-data-dir=/tmp/skeptic-profile");
        guard.remove("SKEPTIC_STATE");
        guard.remove("SKEPTIC_SESSION_NAME");
        guard.remove("SKEPTIC_CDP");
        guard.remove("SKEPTIC_AUTO_CONNECT");
        guard.remove("SKEPTIC_PROVIDER");
    ...
    async fn test_allowed_domains_reject_env_restore_during_auto_launch() {
        ...
        guard.set("SKEPTIC_ALLOWED_DOMAINS", "example.com");
        guard.set("SKEPTIC_SESSION_NAME", "saved-session");
        guard.remove("SKEPTIC_PROFILE");
        guard.remove("SKEPTIC_ARGS");
        guard.remove("SKEPTIC_STATE");
        guard.remove("SKEPTIC_CDP");
    ...
    async fn test_allowed_domains_reject_env_storage_state_during_auto_launch() {
        ...
        guard.set("SKEPTIC_ALLOWED_DOMAINS", "example.com");
        guard.set("SKEPTIC_STATE", "/tmp/skeptic-state.json");
        guard.remove("SKEPTIC_PROFILE");
        guard.remove("SKEPTIC_ARGS");
        guard.remove("SKEPTIC_SESSION_NAME");
        guard.remove("SKEPTIC_CDP");
    ...
    async fn test_allowed_domains_reject_plugin_profile_args() {
        ...
        fs::write(
            ...
            r#"#!/bin/sh
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"launch":{"args":["--user-data-dir=/tmp/plugin-profile"]}}'
"#,
        )
        .unwrap();
        let mut perms = fs::metadata(&plugin_path).unwrap().permissions();
    ...
    async fn test_allowed_domains_reject_direct_page_provider_plugins() {
        ...
        fs::write(
            ...
            r#"#!/bin/sh
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"browser":{"cdpUrl":"ws://127.0.0.1:9222/devtools/page/test","directPage":true}}'
"#,
        )
        .unwrap();
        let mut perms = fs::metadata(&plugin_path).unwrap().permissions();
    ...
    fn test_write_extensions_file_from_paths_uses_final_extensions() {
        let guard = EnvGuard::new(&["SKEPTIC_SOCKET_DIR", "SKEPTIC_EXTENSIONS"]);
        let dir = tempfile::tempdir().unwrap();
        guard.set("SKEPTIC_SOCKET_DIR", dir.path().to_str().unwrap());
        guard.set("SKEPTIC_EXTENSIONS", "/env/ext");
        let extensions = vec![
    ...
    fn test_har_entry_to_json_enriches_request_and_response() {
        ...
        assert_eq!(har["request"]["postData"]["text"], "[REDACTED]");
        assert_eq!(har["request"]["cookies"][0]["name"], "session");
        assert_eq!(har["request"]["cookies"][0]["value"], "[REDACTED]");
        assert_eq!(har["request"]["cookies"][1]["name"], "theme");
        assert_eq!(har["request"]["cookies"][1]["value"], "[REDACTED]");
        ...
        assert_eq!(har["response"]["cookies"][0]["value"], "[REDACTED]");
        assert_eq!(har["_skepticRedaction"], "redacted");
    ...
    async fn test_handle_har_stop_without_path_uses_default_location() {
        ...
        assert_eq!(har["log"]["creator"]["name"], "skeptic");
    ...
    async fn test_execute_har_stop_skips_browser_auto_launch() {
        let path =
            std::env::temp_dir().join(format!("skeptic-har-stop-{}.har", unix_timestamp_millis()));
        let mut state = DaemonState::new();
        state.har_entries.push(HarEntry {
    ...
    fn test_default_timeout_ms_from_env() {
        let env = EnvGuard::new(&["SKEPTIC_DEFAULT_TIMEOUT"]);
        // When SKEPTIC_DEFAULT_TIMEOUT is set, DaemonState should use it
        env.set("SKEPTIC_DEFAULT_TIMEOUT", "3000");
        let state = DaemonState::new();
    ...
    fn test_default_timeout_ms_fallback() {
        let env = EnvGuard::new(&["SKEPTIC_DEFAULT_TIMEOUT"]);
        // When SKEPTIC_DEFAULT_TIMEOUT is unset, DaemonState uses the
        // documented 25s default (below the CLI's 30s IPC read timeout).
        env.remove("SKEPTIC_DEFAULT_TIMEOUT");
        let state = DaemonState::new();
    ...
    async fn test_credentials_roundtrip_via_actions() {
        ...
        let key_var = "SKEPTIC_ENCRYPTION_KEY";
    ...
    async fn test_auto_dialog_enabled_by_default() {
        let guard = EnvGuard::new(&["SKEPTIC_NO_AUTO_DIALOG"]);
        std::env::remove_var("SKEPTIC_NO_AUTO_DIALOG");
        let state = DaemonState::new();
    ...
    async fn test_auto_dialog_disabled_by_env() {
        let guard = EnvGuard::new(&["SKEPTIC_NO_AUTO_DIALOG"]);
        guard.set("SKEPTIC_NO_AUTO_DIALOG", "1");
        let state = DaemonState::new();
        assert!(
            !state.auto_dialog,
            "auto_dialog should be false when SKEPTIC_NO_AUTO_DIALOG=1"
        );
        drop(guard);
    }
    ...
    async fn test_auto_dialog_disabled_by_env_true() {
        let guard = EnvGuard::new(&["SKEPTIC_NO_AUTO_DIALOG"]);
        guard.set("SKEPTIC_NO_AUTO_DIALOG", "true");
        let state = DaemonState::new();
        assert!(
            !state.auto_dialog,
            "auto_dialog should be false when SKEPTIC_NO_AUTO_DIALOG=true"
        );
        drop(guard);
    }
    ...
    async fn test_auto_dialog_not_disabled_by_random_value() {
        let guard = EnvGuard::new(&["SKEPTIC_NO_AUTO_DIALOG"]);
        guard.set("SKEPTIC_NO_AUTO_DIALOG", "no");
        let state = DaemonState::new();
        assert!(
            state.auto_dialog,
