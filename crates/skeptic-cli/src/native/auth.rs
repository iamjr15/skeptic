...
fn get_auth_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".skeptic").join("auth")
    } else {
        std::env::temp_dir().join("skeptic").join("auth")
    }
}
...
const ENCRYPTION_KEY_ENV: &str = "SKEPTIC_ENCRYPTION_KEY";
const KEY_FILE_NAME: &str = ".encryption-key";

fn get_skeptic_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".skeptic")
    } else {
...
fn get_key_file_path() -> PathBuf {
    get_skeptic_dir().join(KEY_FILE_NAME)
}
...
/// Read the encryption key from SKEPTIC_ENCRYPTION_KEY env var or
/// ~/.skeptic/.encryption-key file (matching the Node.js implementation).
...
fn ensure_encryption_key() -> Result<Vec<u8>, String> {
    ...
    let _ = writeln!(
        std::io::stderr(),
        "[skeptic] Auto-generated encryption key at {} -- back up this file or set {}",
        key_file.display(),
        ENCRYPTION_KEY_ENV
    );
