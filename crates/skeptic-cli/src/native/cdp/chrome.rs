...
/// instead of failing. Opt out with SKEPTIC_NO_XVFB=1.
///
...
fn maybe_start_xvfb(options: &LaunchOptions) -> Option<XvfbServer> {
    ...
    let auth_file = std::env::temp_dir().join(format!("skeptic-xauth-{}", uuid::Uuid::new_v4()));
...
pub struct LaunchOptions {
    ...
    /// hosts (SKEPTIC_NO_XVFB). Carried as a launch option so the
...
fn build_chrome_args(options: &LaunchOptions) -> Result<ChromeArgs, String> {
    ...
    if options.headless && !has_extensions {
        ...
        // presence-based, so skeptic exposes --hide-scrollbars false
    ...
    let (user_data_dir, temp_user_data_dir) = if let Some(ref profile) = options.profile {
        ...
        let dir = std::env::temp_dir().join(format!("skeptic-chrome-{}", uuid::Uuid::new_v4()));
        ...
        None => find_chrome().ok_or_else(|| {
            ...
            format!(
                "Chrome not found. Checked:\n  \
                 ...
                 Run `skeptic install` to download Chrome, or use --executable-path.",
                cache_dir.display()
            )
        })?,
    };
...
pub fn find_chrome() -> Option<PathBuf> {
    // 1. Check Chrome downloaded by `skeptic install`
    ...
    if cache_dir.exists() {
        let _ = writeln!(
            ...
            "Warning: Chrome cache directory exists ({}) but no Chrome binary found inside. \
             Falling back to system Chrome. Run `skeptic install` to re-download.",
            cache_dir.display()
        );
    }
...
pub fn copy_chrome_profile(
    ...
    let temp_dir = std::env::temp_dir().join(format!("skeptic-profile-{}", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    fn test_build_args_hide_scrollbars_false_suppresses_default_hide_scrollbars() {
        ...
        assert!(
            ...
            "--hide-scrollbars false should suppress skeptic's default hide switch"
        );
        if let Some(ref dir) = result.temp_user_data_dir {
            let _ = std::fs::remove_dir_all(dir);
        }
    ...
    fn test_chrome_process_drop_cleans_temp_dir() {
        let dir =
            std::env::temp_dir().join(format!("skeptic-chrome-drop-test-{}", uuid::Uuid::new_v4()));
