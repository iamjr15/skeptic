...
fn cleanup_restore_state_files(restore_key: &str) {
    let Some(sessions_dir) = dirs::home_dir().map(|home| home.join(".skeptic/sessions")) else {
        return;
    };
...
async fn e2e_lightpanda_auto_launch_can_open_page() {
    ...
    let prev_engine = std::env::var("SKEPTIC_ENGINE").ok();
    let prev_path = std::env::var("SKEPTIC_EXECUTABLE_PATH").ok();
    std::env::set_var("SKEPTIC_ENGINE", "lightpanda");
    std::env::set_var("SKEPTIC_EXECUTABLE_PATH", &lightpanda_bin);
    ...
    match prev_engine {
        Some(value) => std::env::set_var("SKEPTIC_ENGINE", value),
        None => std::env::remove_var("SKEPTIC_ENGINE"),
    }
    match prev_path {
        Some(value) => std::env::set_var("SKEPTIC_EXECUTABLE_PATH", value),
        None => std::env::remove_var("SKEPTIC_EXECUTABLE_PATH"),
    }
...
async fn e2e_screenshot() {
    ...
    let tmp_path = std::env::temp_dir()
        .join("skeptic-e2e-test-screenshot.png")
        .to_string_lossy()
        .to_string();
    let resp = execute_command(
    ...
    let resp = execute_command(
        &json!({
            "id": "7",
            "action": "evaluate",
            "script": "document.getElementById('__skeptic_annotations__') === null"
        }),
        &mut state,
    )
    .await;
...
async fn e2e_save_state_cross_domain() {
    ...
    let tmp_state = std::env::temp_dir()
        .join("skeptic-e2e-cross-domain-state.json")
        .to_string_lossy()
        .to_string();
    let resp = execute_command(
...
async fn e2e_profile_cookie_persistence() {
    let profile_dir =
        std::env::temp_dir().join(format!("skeptic-e2e-profile-{}", uuid::Uuid::new_v4()));
...
// skeptic (e.g. by the user or another CDP client) should be detected
...
async fn e2e_externally_opened_tab_detected() {
    ...
    // opens a tab while skeptic is connected via --cdp.
...
///
/// This is the e2e equivalent of `skeptic --state ./auth.json open <url>`.
...
async fn e2e_state_flag_missing_file_fails_launch() {
    ...
    let missing_path = std::env::temp_dir()
        .join(format!(
            "skeptic-e2e-missing-state-{}.json",
            uuid::Uuid::new_v4()
        ))
        .to_string_lossy()
        .to_string();
...
async fn e2e_storage_state_launch_restarts_clean_browser() {
    let state_one = std::env::temp_dir()
        .join(format!(
            "skeptic-e2e-storage-reuse-1-{}.json",
            uuid::Uuid::new_v4()
        ))
        .to_string_lossy()
        .to_string();
    let state_two = std::env::temp_dir()
        .join(format!(
            "skeptic-e2e-storage-reuse-2-{}.json",
            uuid::Uuid::new_v4()
        ))
        .to_string_lossy()
        .to_string();
...
/// Verify that SKEPTIC_STATE env var restores cookies at auto-launch
...
async fn e2e_state_env_restores_cookies_on_auto_launch() {
    ...
    // Session 2: set SKEPTIC_STATE env var and let auto_launch pick it
    // up. No explicit `launch` command — just navigate, which triggers
    // auto_launch internally.
    {
        let env = EnvGuard::new(&["SKEPTIC_STATE"]);
        env.set("SKEPTIC_STATE", &state_path);
        ...
        assert!(
            found,
            "Cookie should be restored via SKEPTIC_STATE env on auto-launch. \
             Cookies found: {:?}",
            cookies
                .iter()
                .map(|c| c["name"].as_str().unwrap_or("?"))
...
async fn e2e_session_name_auto_restores_cookies() {
    ...
    let env = EnvGuard::new(&["SKEPTIC_SESSION_NAME"]);
    env.set("SKEPTIC_SESSION_NAME", &session_name);
    ...
    let sessions_dir = dirs::home_dir().unwrap().join(".skeptic").join("sessions");
    if let Ok(entries) = std::fs::read_dir(&sessions_dir) {
        for entry in entries.flatten() {
            let fname = entry.file_name().to_string_lossy().to_string();
...
async fn e2e_restore_loads_during_explicit_launch_before_navigation() {
    ...
    let env = EnvGuard::new(&[
        ...
        "SKEPTIC_RESTORE_SAVE",
        "SKEPTIC_STATE",
        "SKEPTIC_ENCRYPTION_KEY",
    ]);
    env.remove("SKEPTIC_SESSION_NAME");
    env.remove("SKEPTIC_RESTORE_SAVE");
    env.remove("SKEPTIC_STATE");
    env.remove("SKEPTIC_ENCRYPTION_KEY");
...
async fn e2e_explicit_state_load_restores_cookies() {
    let state_path = std::env::temp_dir()
        .join(format!(
            "skeptic-e2e-explicit-load-{}.json",
            uuid::Uuid::new_v4()
        ))
        .to_string_lossy()
        .to_string();
...
async fn e2e_react_tree_with_enable_hook() {
    let guard = EnvGuard::new(&["SKEPTIC_ENABLE"]);
    guard.set("SKEPTIC_ENABLE", "react-devtools");
    let mut state = DaemonState::new();
