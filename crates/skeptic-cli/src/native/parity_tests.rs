...
fn minimal_command(action: &str, id: &str, state_path: &str) -> Value {
    ...
    match action {
        ...
        "state_save" | "state_load" | "state_show" | "state_clear" => {
            obj.insert("path".to_string(), json!(state_path));
        }
        "state_rename" => {
            obj.insert("path".to_string(), json!(state_path));
...
async fn test_all_documented_actions_are_handled() {
    ...
    let state_dir = tempfile::tempdir().expect("create isolated parity-test state directory");
    let state_path = state_dir.path().join("state.json");
    let state_path = state_path.to_string_lossy();
    let mut state = DaemonState::new();
    state.default_timeout_ms = 100;

    for (i, action) in DOCUMENTED_ACTIONS.iter().enumerate() {
        let id = format!("parity-{}", i);
        let cmd = minimal_command(action, &id, &state_path);
