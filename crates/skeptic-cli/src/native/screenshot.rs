...
const ANNOTATION_OVERLAY_ID: &str = "__skeptic_annotations__";
...
fn get_screenshot_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".skeptic").join("tmp").join("screenshots")
    } else {
        std::env::temp_dir().join("skeptic").join("screenshots")
    }
}
