...
pub async fn take_snapshot(
    ...
    if let Some(frame_id) = frame_id {
        visited.insert(frame_id.to_string());
    }
    Box::pin(take_snapshot_inner(
        client,
        session_id,
        options,
        ref_map,
...
async fn take_snapshot_inner(
    ...
    iframe_sessions: &HashMap<String, String>,
    frame_depth: usize,
    visited_frames: &mut HashSet<String>,
) -> Result<String, String> {
    client
    ...
    // Recurse through nested same- and cross-origin frames. A hard depth cap
    // and frame-id cycle set protect malformed/rapidly mutating frame trees.
    if frame_depth < 16 {
        ...
        for node in tree_nodes.iter() {
            ...
            {
                if !visited_frames.insert(child_fid.clone()) {
                    continue;
                }
...
"#;
    ...
    if let Err(e) = client
        ...
        .await
    {
        eprintln!("[skeptic] Warning: failed to clean up data-__ab-ci attributes: {e}");
    }
