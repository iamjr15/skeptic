...
pub async fn save_state(
    ...
    if let Ok(key) = std::env::var("SKEPTIC_ENCRYPTION_KEY") {
...
pub async fn save_auto_state_transactional(
    ...
    let final_path = if std::env::var("SKEPTIC_ENCRYPTION_KEY").is_ok() {
...
fn read_state_json(path: &str) -> Result<String, String> {
    if is_encrypted_state(std::path::Path::new(path)) {
        let key = std::env::var("SKEPTIC_ENCRYPTION_KEY")
            .map_err(|_| "Encrypted state file requires SKEPTIC_ENCRYPTION_KEY".to_string())?;
        let data =
...
/// Return the skeptic state root (`~/.skeptic`, falling back to
/// `<tempdir>/skeptic` when the home directory can't be resolved).
/// This is the parent of `sessions/`, auth storage, and the encryption key.
pub fn get_state_dir() -> PathBuf {
    let base = if let Some(home) = dirs::home_dir() {
        home.join(".skeptic")
    } else {
        std::env::temp_dir().join("skeptic")
    };

    if let Ok(namespace) = std::env::var("SKEPTIC_NAMESPACE") {
...
mod tests {
    ...
    fn test_state_show_nonexistent_file() {
        let result = state_show("/tmp/nonexistent-skeptic-state-file.json");
        assert!(result.is_err());
    }

    #[test]
    fn test_state_clear_nonexistent_file() {
        let result = state_clear(Some("/tmp/nonexistent-skeptic-state-file.json"));
        assert!(result.is_err());
    }
    ...
    fn test_state_clear_removes_transactional_backups() {
        let guard = crate::test_utils::EnvGuard::new(&["HOME", "SKEPTIC_NAMESPACE"]);
    ...
    fn test_state_rename_nonexistent() {
        let result = state_rename("/tmp/nonexistent-skeptic-state-file.json", "new-name");
        assert!(result.is_err());
    ...
    fn test_get_state_dir_namespace_scopes_sessions() {
        let _guard = crate::test_utils::EnvGuard::new(&["SKEPTIC_NAMESPACE"]);
        _guard.set("SKEPTIC_NAMESPACE", "Worktree: One");

        let dir = get_state_dir();
        let expected_state_suffix = PathBuf::from(".skeptic")
            .join("namespaces")
            .join("worktree-one")
            .join("state");
