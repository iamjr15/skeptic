...
//! Core skeptic keeps ownership of browser automation, policy checks, and
...
pub const PROTOCOL_VERSION: &str = "skeptic.plugin.v1";
...
fn derive_plugin_name(source: &PluginSource) -> String {
    ...
    raw.strip_prefix("skeptic-plugin-")
...
fn config_path_for_scope(scope: &PluginConfigScope) -> Result<PathBuf, String> {
    match scope {
        PluginConfigScope::Project => Ok(PathBuf::from("skeptic.json")),
        PluginConfigScope::Global => dirs::home_dir()
            .map(|d| d.join(".skeptic").join("config.json"))
    ...
    match sub {
        ...
        "run" => {
            ...
            if is_core_plugin_entrypoint(request_type) {
                print_plugin_error(
                    &format!(
                        "plugin run cannot invoke core plugin entrypoint '{}'; use the dedicated skeptic command path",
                        request_type
                    ),
                    json_output,
                );
...
mod tests {
    ...
    fn plugin_add_source_detection_matches_reference_shape() {
        let npm = parse_plugin_source("skeptic-plugin-captcha").unwrap();
        assert_eq!(npm.kind, PluginSourceKind::Npm);
        assert_eq!(npm.install_spec, "skeptic-plugin-captcha");
        assert_eq!(npm.source, "npm:skeptic-plugin-captcha");
        assert_eq!(derive_plugin_name(&npm), "captcha");

        let scoped = parse_plugin_source("@acme/skeptic-plugin-vault").unwrap();
        assert_eq!(scoped.kind, PluginSourceKind::Npm);
        assert_eq!(scoped.install_spec, "@acme/skeptic-plugin-vault");
        assert_eq!(scoped.source, "npm:@acme/skeptic-plugin-vault");
        assert_eq!(derive_plugin_name(&scoped), "vault");

        let github = parse_plugin_source("iamjr15/skeptic-plugin-browserbox").unwrap();
        assert_eq!(github.kind, PluginSourceKind::Github);
        assert_eq!(
            github.install_spec,
            "github:iamjr15/skeptic-plugin-browserbox"
        );
        assert_eq!(github.source, "github:iamjr15/skeptic-plugin-browserbox");
    ...
    fn plugin_add_builds_config_from_manual_capabilities() {
        let args = vec![
            "plugin".to_string(),
            "add".to_string(),
            "skeptic-plugin-captcha".to_string(),
        ...
        assert_eq!(
            plugin.args,
            vec!["-y".to_string(), "skeptic-plugin-captcha".to_string()]
        );
        assert_eq!(
            plugin.capabilities,
            vec!["command.run".to_string(), "captcha.solve".to_string()]
        );
        assert_eq!(plugin.source.as_deref(), Some("npm:skeptic-plugin-captcha"));
    }

    #[test]
    fn plugin_config_upsert_replaces_existing_plugin() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("skeptic.json");
        fs::write(
            &path,
        ...
        let plugin = PluginConfig {
            name: "captcha".to_string(),
            command: "npx".to_string(),
            args: vec!["-y".to_string(), "skeptic-plugin-captcha".to_string()],
            capabilities: vec!["command.run".to_string(), "captcha.solve".to_string()],
            source: Some("npm:skeptic-plugin-captcha".to_string()),
        };
        ...
        assert_eq!(plugins[0]["source"], "npm:skeptic-plugin-captcha");
    ...
    async fn resolve_credential_uses_configured_stdio_plugin() {
        ...
        let _guard = EnvGuard::new(&["SKEPTIC_PLUGINS"]);
        let dir = tempfile::tempdir().unwrap();
...
"#,
        ...
        .unwrap();
        _guard.set("SKEPTIC_PLUGINS", &registry);
    ...
    async fn credential_plugin_failure_does_not_echo_plugin_error() {
        ...
        std::fs::write(
            ...
            r#"#!/bin/sh
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":false,"error":"secret-token-value"}'
"#,
        )
        .unwrap();
        let mut perms = std::fs::metadata(&plugin_path).unwrap().permissions();
    ...
    async fn browser_provider_plugin_returns_cdp_connection() {
        ...
        std::fs::write(
            ...
            r#"#!/bin/sh
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"browser":{"cdpUrl":"ws://127.0.0.1:9222/devtools/browser/test","directPage":true,"metadata":{"sessionId":"s1"},"cleanup":{"sessionId":"s1"}}}'
"#,
        )
        .unwrap();
        let mut perms = std::fs::metadata(&plugin_path).unwrap().permissions();
    ...
    async fn launch_mutator_plugin_returns_launch_changes() {
        ...
        std::fs::write(
            ...
            r#"#!/bin/sh
cat >/dev/null
printf '%s' '{"protocol":"skeptic.plugin.v1","success":true,"launch":{"args":["--disable-blink-features=AutomationControlled"],"extensions":["/tmp/ext"],"initScripts":["Object.defineProperty(navigator,\"webdriver\",{get:()=>undefined});"],"userAgent":"plugin-agent"}}'
"#,
        )
        .unwrap();
        let mut perms = std::fs::metadata(&plugin_path).unwrap().permissions();
