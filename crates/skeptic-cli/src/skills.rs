...
const EMBEDDED_SKILL_NAMES: &[&str] = &["core", "doctor", "evidence", "mobile"];

fn embedded_skill(name: &str) -> Option<String> {
    EmbeddedSkills::get(&format!("{name}/SKILL.md"))
        .and_then(|asset| String::from_utf8(asset.data.into_owned()).ok())
}

fn embedded_references(name: &str) -> Vec<(String, String)> {
    let prefix = format!("{name}/references/");
    let mut references = EmbeddedSkills::iter()
        .filter_map(|path| {
            let path = path.as_ref();
            path.strip_prefix(&prefix).and_then(|relative| {
                EmbeddedSkills::get(path).and_then(|asset| {
                    String::from_utf8(asset.data.into_owned())
                        .ok()
                        .map(|content| (format!("references/{relative}"), content))
                })
            })
        })
        .collect::<Vec<_>>();
...
pub fn embedded_skill_catalog() -> Vec<serde_json::Value> {
    EMBEDDED_SKILL_NAMES
        .iter()
        .filter_map(|name| {
            let content = embedded_skill(name)?;
...
pub fn embedded_skill_bundle(name: &str, full: bool) -> Option<serde_json::Value> {
    let mut value = json!({
        "name": name,
        "content": embedded_skill(name)?,
        "version": env!("CARGO_PKG_VERSION"),
        "embedded": true
    });
    if full {
        value["files"] = json!(embedded_references(name)
            .into_iter()
...
pub fn install_embedded_skills(destination: &Path, names: &[&str]) -> Result<Vec<PathBuf>, String> {
    let mut installed = Vec::new();
    for name in names {
        let content =
            embedded_skill(name).ok_or_else(|| format!("embedded skill `{name}` is missing"))?;
        let skill_dir = destination.join(name);
        if skill_dir.exists() {
            return Err(format!("{} already exists", skill_dir.display()));
        }
        fs::create_dir_all(skill_dir.join("references")).map_err(|error| error.to_string())?;
        fs::write(skill_dir.join("SKILL.md"), content).map_err(|error| error.to_string())?;
        for (relative, content) in embedded_references(name) {
...
fn run_embedded_list(json_mode: bool) {
    let items = embedded_skill_catalog();
    if json_mode {
        println!(
            "{}",
        ...
        for item in items {
            println!(
                ...
                truncate_description(item["description"].as_str().unwrap_or_default(), 70)
            );
        }
    }
}

fn run_embedded_get(names: &[String], get_all: bool, full: bool, json_mode: bool) {
    let selected = if get_all {
        EMBEDDED_SKILL_NAMES
            .iter()
    ...
    for name in &selected {
        if embedded_skill(name).is_none() {
    ...
    if json_mode {
        let data = selected
            .iter()
            .filter_map(|name| embedded_skill_bundle(name, full))
            .collect::<Vec<_>>();
        println!(
            "{}",
            serde_json::to_string(&json!({"success":true,"data":data})).unwrap_or_default()
        );
    } else {
        for (index, name) in selected.iter().enumerate() {
            ...
            print!("{}", embedded_skill(name).unwrap_or_default());
            if full {
                for (path, content) in embedded_references(name) {
                    println!("\n--- {path} ---\n{content}");
                }
            }
        }
...
///
/// Resolution order:
/// 1. SKEPTIC_SKILLS_DIR env var (points directly at a single directory)
...
fn find_package_root() -> Option<PathBuf> {
    if let Ok(exe) = env::current_exe() {
        ...
        if let Some(parent) = exe.parent() {
            // npm install layout: bin/skeptic-* -> ../
...
fn find_skills_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = env::var("SKEPTIC_SKILLS_DIR") {
    ...
    if targets.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "No skill name provided. Usage: skeptic skills get <name>",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} No skill name provided. Usage: skeptic skills get <name>",
...
pub fn run_skills(args: &[String], json_mode: bool) {
    ...
    if matches!(subcommand, None | Some("list")) {
        run_embedded_list(json_mode);
        return;
    }
    if subcommand == Some("get") {
        let names: Vec<String> = args[2..]
            .iter()
            .filter(|arg| *arg != "--full" && *arg != "--all")
            .cloned()
            .collect();
        let get_all = args[2..].iter().any(|arg| arg == "--all");
        if get_all
            || names
                .iter()
                .all(|name| EMBEDDED_SKILL_NAMES.contains(&name.as_str()))
        {
            run_embedded_get(
                &names,
                get_all,
                args[2..].iter().any(|arg| arg == "--full"),
                json_mode,
            );
            return;
        }
    ...
    if skills_dirs.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "Skills directory not found. Set SKEPTIC_SKILLS_DIR or reinstall via npm.",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} Skills directory not found. Set SKEPTIC_SKILLS_DIR or reinstall via npm.",
                color::error_indicator()
            );
        }
        exit(1);
