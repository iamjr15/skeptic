...
const NPM_REGISTRY_URL: &str = "https://registry.npmjs.org/skeptic/latest";
...
fn detect_install_method() -> InstallMethod {
    if let Ok(exe) = std::env::current_exe() {
        ...
        if path_str.contains("/Cellar/skeptic/")
        ...
        if path_str.contains("node_modules/skeptic") || path_str.contains("node_modules\\skeptic") {
    ...
    {
        if command_succeeds("brew", &["list", "skeptic"]) {
            return InstallMethod::Homebrew;
        }
    }

    if command_output_contains("pnpm", &["list", "-g", "skeptic", "--depth=0"], "skeptic") {
        return InstallMethod::Pnpm;
    }

    if command_output_contains("yarn", &["global", "list", "--depth=0"], "skeptic") {
        return InstallMethod::Yarn;
    }

    if command_output_contains("bun", &["pm", "ls", "-g"], "skeptic") {
        return InstallMethod::Bun;
    }

    if command_succeeds("npm", &["list", "-g", "skeptic", "--depth=0"]) {
        return InstallMethod::Npm;
    }
...
fn run_upgrade_command(method: &InstallMethod) -> bool {
    let (cmd, args, display): (&str, &[&str], &str) = match method {
        InstallMethod::Npm => (
            "npm",
            &["install", "-g", "skeptic@latest"],
            "npm install -g skeptic@latest",
        ),
        InstallMethod::Pnpm => (
            "pnpm",
            &["add", "-g", "skeptic@latest"],
            "pnpm add -g skeptic@latest",
        ),
        ...
        InstallMethod::Yarn => (
            "yarn",
            &["global", "add", "skeptic@latest"],
            "yarn global add skeptic@latest",
        ),
        InstallMethod::Bun => (
            "bun",
            &["install", "-g", "skeptic@latest"],
            "bun install -g skeptic@latest",
        ),
        InstallMethod::Homebrew => ("brew", &["upgrade", "skeptic"], "brew upgrade skeptic"),
        InstallMethod::Cargo => (
            "cargo",
            &["install", "skeptic", "--force"],
            "cargo install skeptic --force",
        ),
...
pub fn run_upgrade() {
    ...
    if !latest.is_empty() && current == latest.as_str() {
        println!(
            "{} skeptic is already at the latest version (v{})",
    ...
    if matches!(method, InstallMethod::Unknown) {
        ...
        eprintln!("    npm install -g skeptic@latest       # npm");
        eprintln!("    pnpm add -g skeptic@latest          # pnpm");
        eprintln!("    yarn global add skeptic@latest       # yarn");
        eprintln!("    bun install -g skeptic@latest        # bun");
        eprintln!("    brew upgrade skeptic                 # Homebrew");
        eprintln!("    cargo install skeptic --force        # Cargo");
        exit(1);
    }
    ...
    if !latest.is_empty() {
        println!(
            "{}",
            color::cyan(&format!("Upgrading skeptic... v{} → v{}", current, latest))
        );
    } else {
        println!(
            "{}",
            color::cyan(&format!("Upgrading skeptic (v{})...", current))
        );
    }
