...
### 31. Daemon mode is opt-out via `--no-daemon`; isolation lives at the BrowserContext layer, not the Browser layer
**Files:** `cli/src/daemon/{client,auto-spawn,lifecycle,rpc,socket}.ts`, `cli/src/runner/worker.ts`, `cli/src/commands/{run,inspect,daemon}.ts`
**Pattern:** B10's persistent `BrowserServer` daemon defaults to **on** for `run` and `inspect` only — every other command (test, generate, validate, mcp, acp, init, ai) keeps the pre-B10 fresh-launch path. The daemon is a control-plane RPC over a Unix socket at `~/.skeptic/daemon.sock` (handshake / version probe / idle reset). Browser, Page, Locator, and routing operations stay on the **direct Playwright WebSocket** — the daemon does not marshal them and does not track contexts. Each test connects via `chromium.connect(wsEndpoint)` and creates its own `BrowserContext` (cookies, storage, service workers, IndexedDB scoped per-test). When the worker closes its WebSocket, Playwright's native disconnect-cleanup tears down that worker's context. We rely on this — there is no skeptic-side context registry. Two consequences worth preserving: (1) state isolation is at the **Context** layer, not the Browser layer — adding "shared cookies across tests" means changing the worker, not the daemon; (2) "the daemon leaked state" is a misdiagnosis we should reject — if state crosses tests, the worker reused a context, not the Browser. The `--no-daemon` flag is the safety valve when the daemon misbehaves; recovery is `skeptic daemon stop && rm -rf ~/.skeptic`. Pre-warming runs in the **main process**, not the worker (`prewarmDaemonIfNeeded` in `auto-spawn.ts`) — a `worker_thread` would resolve `dist/worker.mjs` and mis-spawn the worker entrypoint as a "daemon."
**Rule:** When asked to add cross-test browser state (shared session, sticky cookies), change the worker's context-creation, not the daemon. The daemon is single-purpose: keep the Browser warm. Cleanup-on-disconnect is Playwright's native behavior — do not reinvent it inside the daemon, and do not blame the daemon when contexts misbehave. `--no-daemon` is the per-call escape hatch; `daemon stop` + clearing `~/.skeptic` is the recovery escalation.
